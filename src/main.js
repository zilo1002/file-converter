...
function renderGitHubPage() {
  app.innerHTML = '<div class="min-h-screen bg-slate-950 view-enter">'+
    ...
    '</header>'+
    '<main class="max-w-3xl mx-auto px-4 py-6 space-y-6">'+
      '<div class="rounded-2xl bg-slate-800/20 border border-slate-700/30 p-6 space-y-4">'+
        '<div class="space-y-2">'+
          '<label class="text-sm font-medium text-slate-300">'+t('githubUrlLabel')+'</label>'+
          '<div class="flex gap-2">'+
            '<input type="text" id="githubUrl" placeholder="'+t('githubUrlHint')+'" class="flex-1 bg-slate-800 border border-slate-600 rounded-xl px-4 py-2.5 text-sm text-slate-200 focus:border-purple-500 focus:outline-none placeholder-slate-500">'+
            '<button onclick="window.pasteGitHubUrl()" class="px-4 py-2.5 rounded-xl bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors text-sm font-medium">'+t('paste')+'</button>'+
          '</div>'+
          '<p class="text-[10px] text-slate-500">'+t('githubUrlHint')+'</p>'+
        '</div>'+
        '<button id="githubDownloadBtn" onclick="window.startGitHubDownload()" class="w-full py-3 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-400 hover:to-purple-500 text-white font-semibold rounded-xl shadow-lg shadow-purple-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">'+
          '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>'+
          t('download')+
        '</button>'+
        '<div id="githubProgress" class="hidden space-y-2">'+
          '<div class="h-1 bg-slate-700 rounded-full overflow-hidden">'+
            '<div id="githubProgressBar" class="h-full bg-gradient-to-r from-purple-500 to-purple-400 transition-all duration-300" style="width:0%"></div>'+
          '</div>'+
          '<p id="githubProgressText" class="text-xs text-slate-400 text-center"></p>'+
        '</div>'+
        '<div id="githubRateInfo" class="flex items-center justify-between text-[10px] text-slate-500">'+
          '<span id="githubRateText">'+t('githubRateHint').replace('{remain}', '?').replace('{limit}', '?')+'</span>'+
        '</div>'+
      '</div>'+
      '<div class="rounded-2xl bg-slate-800/20 border border-slate-700/30 p-5">'+
        '<h3 class="text-sm font-semibold mb-3 flex items-center gap-2">'+
        ...
        '<ul class="space-y-2 text-xs text-slate-400">'+
          '<li class="flex items-start gap-2"><span class="text-purple-400 mt-0.5">•</span><span>'+t('githubNote1')+'</span></li>'+
          '<li class="flex items-start gap-2"><span class="text-purple-400 mt-0.5">•</span><span>'+t('githubNote2')+'</span></li>'+
          '<li class="flex items-start gap-2"><span class="text-purple-400 mt-0.5">•</span><span>'+t('githubNote3')+'</span></li>'+
          '<li class="flex items-start gap-2"><span class="text-purple-400 mt-0.5">•</span><span>'+t('githubNote4')+'</span></li>'+
        '</ul>'+
      '</div>'+
    '</main>'+
  '</div>'+
  ...
  checkRateLimit().then(r => {
    if (r) {
      const text = t('githubRateHint').replace('{remain}', r.remaining).replace('{limit}', r.limit);
      const el = document.getElementById('githubRateText');
      if (el) {
        el.textContent = text;
  ...
  if (input) {
    input.addEventListener('paste', e => {
      setTimeout(() => {
        ...
        if (isGitHubUrl(val)) input.value = val;
      }, 0);
    });
    input.focus();
  }
