...
function renderFiles(catId) {
  ...
  const cat = CATEGORIES[catId];
  if (!files.length) { list.classList.add('hidden'); area.classList.add('hidden'); if (batchBar) batchBar.classList.add('hidden'); return; }
  list.classList.remove('hidden'); area.classList.remove('hidden'); if (batchBar) batchBar.classList.remove('hidden');
  ...
  for (let idx = 0; idx < files.length; idx++) {
    ...
    // 自定义输出文件名
    const defaultName = f.name.replace(/\.[^.]+$/i, '.' + (cat.targets[item.target]?.ext || item.target));
    const customNameInput = '<div class="mt-2 flex items-center gap-2">'+
      '<span class="text-[10px] text-slate-500 shrink-0">'+t('customName')+':</span>'+
      '<input type="text" value="'+(item.customName || defaultName)+'" onchange="window.chgCustomName('+idx+',this.value)" class="flex-1 bg-slate-900/50 border border-slate-700 rounded-md px-2 py-1 text-[11px] text-slate-300 focus:border-'+cat.color+'-500 focus:outline-none">'+
    '</div>';
