...
import { parseGitHubUrl, isGitHubUrl, checkRateLimit, downloadGitHubFile, downloadGitHubFolder } from './converters/github.js';
...
async function readABChunked(file) {
  if (file.size <= 50 * 1024 * 1024) return await readAB(file);
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    chunks.push(await readAB(file.slice(start, end)));
  }
...
function updateNetworkStatus() {
  ...
  if (currentCat) renderCategoryPage(currentCat);
}

window.addEventListener('online', updateNetworkStatus);
window.addEventListener('offline', updateNetworkStatus);

window._onSwUpdate = function() {
  toast('🔄', t('updateAvailable'));
  setTimeout(() => {
    if (confirm(t('updateAvailable') + '\n' + t('updateNow') + '?')) {
...
window._onInstallPrompt = function() {
  if (installPromptShown) return;
  installPromptShown = true;
  setTimeout(() => {
    toast('📲', t('installAppDesc'));
  }, 2000);
};
...
function renderHome() {
  ...
  // GitHub 下载卡片
  cards += '<a href="#github" class="group relative overflow-hidden rounded-2xl bg-slate-800/40 border border-slate-700/30 p-6 hover:border-purple-500/40 transition-all duration-300 hover:-translate-y-1">'+
    ...
    '<div class="relative flex items-start justify-between mb-4">'+
      '<div class="w-14 h-14 rounded-2xl bg-purple-500/20 border border-purple-500/20 flex items-center justify-center text-3xl">🐙</div>'+
      ...
      '<div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">'+
        ...
        '<div class="flex items-center gap-3">'+
          '<div class="flex items-center gap-1">'+
            '<button onclick="window.switchTheme()" class="lang-btn" title="'+t('themeAuto')+'">'+(window._theme==='dark'?'🌙':window._theme==='light'?'☀️':'🌓')+'</button>'+
          ...
          '</div>'+
          (window._deferredPrompt ? '<button onclick="window.promptInstall()" class="hidden sm:flex items-center gap-1.5 text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-3 py-1.5 rounded-full hover:bg-emerald-500/20 transition-colors">📲 '+t('installApp')+'</button>' : '')+
...
async function renderCategoryPage(catId) {
  ...
  const matrixData = {
    ...
    pdf: [
      { from: 'PDF', to: t('pdfSplit') + ' / ' + t('pdfMerge') + ' / ' + t('pdfEncrypt'), icon: '📕' }
    ]
  };
  ...
  app.innerHTML = '<div class="min-h-screen bg-slate-950 view-enter">'+
    ...
    '<main class="max-w-3xl mx-auto px-4 py-6 space-y-6">'+
      '<div id="dropZone" class="group relative border-2 border-dashed '+(isOnline?'border-slate-600 hover:border-'+cat.color+'-500/40':'border-red-500/30 bg-red-500/5 cursor-not-allowed')+' rounded-2xl bg-slate-800/20 hover:bg-slate-800/40 active:border-'+cat.color+'-500 transition-all duration-300 p-8 sm:p-12 text-center '+(isOnline?'cursor-pointer':'cursor-not-allowed')+'">'+
        '<input type="file" id="fileInput" multiple class="absolute inset-0 w-full h-full opacity-0 '+(isOnline?'cursor-pointer':'cursor-not-allowed pointer-events-none')+' z-10" accept="'+cat.accept+'" '+(isOnline?'':'disabled')+'>'+
        (isOnline?'':'<div class="absolute inset-0 z-20 flex items-center justify-center bg-slate-900/60 rounded-2xl"><div class="text-center"><div class="text-2xl mb-1">📡</div><p class="text-sm font-medium text-red-400">'+t('offlineMode')+'</p><p class="text-xs text-slate-400 mt-1">'+t('uploadDisabledOffline')+'</p></div></div>')+
      ...
      '<div id="convertArea" class="hidden space-y-3">'+
        '<button id="convertBtn" class="w-full py-3 bg-gradient-to-r from-'+cat.color+'-500 to-'+cat.color+'-600 hover:from-'+cat.color+'-400 hover:to-'+cat.color+'-500 text-white font-semibold rounded-xl shadow-lg shadow-'+cat.color+'-500/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2">'+
          ...
          (catId==='pdf' ? (pdfTool==='split'?t('split'):pdfTool==='merge'?t('merge'):t('applyEncrypt')) : t('startConvert'))+
        '</button>'+
        ...
        '<button onclick="window.clearCompleted()" class="w-full py-2 rounded-xl bg-slate-800/50 border border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-slate-300 transition-colors text-xs font-medium flex items-center justify-center gap-1.5">'+
          ...
          t('clearCompleted')+
        '</button>'+
        '<button onclick="window.exportLog()" class="w-full py-2 rounded-xl bg-slate-800/50 border border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-slate-300 transition-colors text-xs font-medium flex items-center justify-center gap-1.5">'+
...
window.dragDrop = function(idx) {
  if (dragSrcIdx === null || dragSrcIdx === idx) return;
  const item = files[dragSrcIdx];
  files.splice(dragSrcIdx, 1);
...
/* ============================================================
   主题切换
   ============================================================ */
window._theme = localStorage.getItem('formathub-theme') || 'auto';

function applyTheme() {
  const root = document.documentElement;
  const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
  let isDark = window._theme === 'dark' || (window._theme === 'auto' && prefersDark);
...
window.switchTheme = function() {
  ...
  const idx = modes.indexOf(window._theme);
  window._theme = modes[(idx + 1) % modes.length];
  localStorage.setItem('formathub-theme', window._theme);
  applyTheme();
  render();
};
...
function handleFiles(fileList, catId) {
  ...
  for (let f of Array.from(fileList)) {
    ...
    if (!cat.exts.includes(e)) { toast('⚠️', '「'+f.name+'」'+t('notSupported')); continue; }
    // 重名检测
    let finalName = f.name;
    if (existingNames.has(finalName)) {
      const ext = getExt(finalName);
      const base = finalName.slice(0, -(ext.length + 1)) || finalName;
      let counter = 1;
      while (existingNames.has(finalName)) {
        finalName = base + ' (' + counter + ').' + ext;
        counter++;
      }
      f = new File([f], finalName, { type: f.type, lastModified: f.lastModified });
      toast('⚠️', t('renameConflict') + ': ' + finalName);
    }
  ...
  try {
    if (catId === 'image') {
      ...
      if (ext === 'zip') {
        ...
        const text = await readText(item.file);
        const langMap = { json: 'json', xml: 'xml', yaml: 'yaml', yml: 'yaml', toml: 'ini', csv: 'csv' };
        item.preview = { type: 'code', content: text.slice(0, 800), lang: langMap[ext] || 'text' };
      } else {
        item.preview = { type: 'none' };
      }
    }
    // 代码文件高亮预览（跨分类）
    const codeExts = ['js','ts','py','css','scss','less','html','htm','php','java','cpp','c','go','rs','swift','kt','rb','sh','sql','vue','jsx','tsx'];
    if (codeExts.includes(ext)) {
      const text = await readText(item.file);
      const langMap = { js: 'javascript', ts: 'javascript', py: 'python', css: 'css', scss: 'css', less: 'css', html: 'html', htm: 'html', php: 'php', java: 'java', cpp: 'cpp', c: 'c', go: 'go', rs: 'rust', swift: 'swift', kt: 'kotlin', rb: 'ruby', sh: 'bash', sql: 'sql', vue: 'html', jsx: 'javascript', tsx: 'javascript' };
      item.preview = { type: 'code', content: text.slice(0, 800), lang: langMap[ext] || 'text' };
    }
  } catch (e) {
...
function renderPreview(preview, catId) {
  ...
  if (preview.type === 'code') {
    ...
    const langClass = preview.lang ? 'language-' + preview.lang : '';
    setTimeout(() => {
      const el = document.getElementById(codeId);
      if (el && window.Prism) Prism.highlightElement(el);
    }, 50);
    return '<div class="mt-3 rounded-lg overflow-hidden border border-slate-700/30">'+
      '<div class="flex items-center justify-between px-3 py-1.5 bg-slate-800 border-b border-slate-700/30">'+
        '<span class="text-[10px] text-slate-400">'+t('codePreview')+' · '+(preview.lang || 'text')+'</span>'+
      '</div>'+
      '<pre class="m-0 p-3 bg-slate-900/80 overflow-x-auto"><code id="'+codeId+'" class="'+langClass+' text-xs font-mono leading-relaxed">'+escapeHtml(preview.content)+'</code></pre>'+
      '<p class="text-[10px] text-slate-500 px-3 py-1.5 bg-slate-900/30">'+t('previewFirst500')+'</p>'+
    '</div>';
  }
  return '';
}
...
function renderFiles(catId) {
  ...
  let startIdx = 0;
  let endIdx = files.length;
  if (files.length > VISIBLE_COUNT) {
    const scrollTop = list.scrollTop || 0;
    startIdx = Math.max(0, Math.floor(scrollTop / ITEM_HEIGHT) - 2);
    endIdx = Math.min(files.length, startIdx + VISIBLE_COUNT + 4);
  }
  ...
  let html = '';
  if (files.length > VISIBLE_COUNT) {
    html += '<div style="height:' + (startIdx * ITEM_HEIGHT) + 'px"></div>';
  }
  for (let idx = startIdx; idx < endIdx; idx++) {
    ...
    html += '<div draggable="true" ondragstart="window.dragStart(' + idx + ')" ondragover="window.dragOver(event)" ondrop="window.dragDrop(' + idx + ')" class="slide-in rounded-xl bg-slate-800/40 border border-slate-700/40 p-4 ' + (item.status === 'done' ? 'border-emerald-500/20' : '') + ' ' + (item.selected ? 'ring-1 ring-' + cat.color + '-500/30' : '') + '">' +
      '<div class="flex items-center gap-3 mb-3">' +
        '<span class="cursor-grab text-slate-600 hover:text-slate-400 select-none" title="' + t('dragToReorder') + '">⋮⋮</span>' +
      ...
      (item.status === 'converting' ? '<div class="mt-3"><div class="flex items-center justify-between mb-1"><span class="text-[10px] text-' + cat.color + '-400">' + (item._stage || t('converting')) + '</span><span class="text-[10px] text-slate-500">' + item.progress + '%</span></div><div class="h-1 bg-slate-700 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-' + cat.color + '-500 to-' + cat.color + '-400 progress-bar" style="width:' + item.progress + '%"></div></div></div>' : '') +
  ...
  if (files.length > VISIBLE_COUNT) {
    html += '<div style="height:' + ((files.length - endIdx) * ITEM_HEIGHT) + 'px"></div>';
  }
  list.innerHTML = html;
}
...
window.exportLog = function() {
  ...
  if (!done.length) { toast('⚠️', '没有可导出的记录'); return; }
  const success = done.filter(f => f.status === 'done').length;
  const failed = done.filter(f => f.status === 'error').length;
  let log = t('logHeader') + '\n';
  log += '='.repeat(40) + '\n';
  log += t('logGeneratedAt') + ': ' + new Date().toLocaleString() + '\n';
  log += t('logTotalFiles') + ': ' + done.length + '\n';
  log += t('logSuccess') + ': ' + success + '\n';
  log += t('logFailed') + ': ' + failed + '\n';
  log += '='.repeat(40) + '\n\n';
  log += t('logDetails') + ':\n';
  for (const item of done) {
    log += '\n[' + item.status.toUpperCase() + '] ' + item.file.name + '\n';
    log += '  Size: ' + fmtSize(item.file.size) + '\n';
    if (item.convertedSize) log += '  Output: ' + fmtSize(item.convertedSize) + '\n';
    if (item.error) log += '  Error: ' + item.error + '\n';
    log += '---\n';
  }
  const blob = new Blob([log], { type: 'text/plain;charset=utf-8' });
  dlBlob(blob, 'formathub-log-' + Date.now() + '.txt');
  toast('✅', t('exportLog') + ' ' + t('done'));
};

window.showHistory = async function() {
  const records = await getHistory();
  let html = '<div class="min-h-screen bg-slate-950 view-enter">'+
    '<header class="border-b border-slate-800 glass sticky top-0 z-50">'+
      '<div class="max-w-3xl mx-auto px-4 py-3.5 flex items-center gap-3">'+
        '<a href="#" class="p-2 -ml-2 rounded-lg hover:bg-slate-800 text-slate-400 transition-colors">'+
          '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>'+
        '</a>'+
        '<h1 class="text-base font-bold">'+t('history')+'</h1>'+
        '<button onclick="window.clearHistoryUI()" class="ml-auto text-xs px-3 py-1.5 rounded-lg bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors">'+t('clearHistory')+'</button>'+
      '</div>'+
    '</header>'+
    '<main class="max-w-3xl mx-auto px-4 py-6">';
  if (!records.length) {
    html += '<div class="text-center py-20"><div class="text-4xl mb-3">📭</div><p class="text-slate-400">'+t('historyEmpty')+'</p></div>';
  } else {
    html += '<div class="space-y-3">';
    for (const r of records) {
      html += '<div class="rounded-xl bg-slate-800/40 border border-slate-700/40 p-4">'+
        '<div class="flex items-center justify-between mb-2">'+
          '<span class="text-sm font-medium">'+escapeHtml(r.cat)+'</span>'+
          '<span class="text-[10px] text-slate-500">'+new Date(r.time).toLocaleString()+'</span>'+
        '</div>'+
        '<p class="text-xs text-slate-400">'+r.count+t('fileCount')+' · '+r.success+t('done')+' · '+r.failed+t('failed')+'</p>'+
      '</div>';
    }
    html += '</div>';
  }
...
window.clearHistoryUI = async function() {
  await clearHistoryDB();
  toast('🗑️', t('historyCleared'));
  window.showHistory();
};
...
document.addEventListener('keydown', e => {
  // Ctrl+A 全选
  if (e.ctrlKey && e.key === 'a' && currentCat) {
    e.preventDefault();
    window.selectAll();
  }
  // Delete 删除选中
  if (e.key === 'Delete' && currentCat) {
    const toRemove = [];
    for (let i = files.length - 1; i >= 0; i--) {
      if (files[i].selected) toRemove.push(i);
    }
    for (const idx of toRemove) window.rmFile(idx);
  }
  // Enter 开始转换
  if (e.key === 'Enter' && currentCat && !converting) {
    if (currentCat === 'pdf') window.startPdfTool();
    else startConvert(currentCat);
  }
});
...
function renderGitHubPage() {
  ...
  app.innerHTML =
    '<div class="min-h-screen bg-slate-950 view-enter">'+
      ...
      '<main class="max-w-2xl mx-auto px-4 py-8 space-y-6">'+
        '<div class="rounded-2xl bg-slate-800/30 border border-slate-700/30 p-6 space-y-4">'+
          '<div class="space-y-2">'+
            '<label class="text-sm font-medium text-slate-300">'+t('githubUrlLabel')+'</label>'+
            '<div class="flex gap-2">'+
              '<input type="text" id="githubUrlInput" placeholder="https://github.com/owner/repo/blob/main/README.md" '+
                'class="flex-1 bg-slate-800 border border-slate-600 rounded-lg px-4 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:border-purple-500 focus:outline-none">'+
              '<button onclick="window.pasteGitHubUrl()" class="px-4 py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg text-sm font-medium transition-colors shrink-0">'+t('paste')+'</button>'+
            '</div>'+
            '<p class="text-xs text-slate-500">'+t('githubUrlHint')+'</p>'+
          '</div>'+
          '<div id="githubRateLimit" class="hidden rounded-lg bg-amber-500/10 border border-amber-500/20 p-3">'+
            '<p class="text-xs text-amber-400 flex items-center gap-2">'+
              '<span>⚠️</span><span id="githubRateText"></span>'+
            '</p>'+
          '</div>'+
          '<button id="githubDownloadBtn" onclick="window.startGitHubDownload()" class="w-full py-3 bg-gradient-to-r from-purple-500 to-purple-600 text-white font-semibold rounded-xl shadow-lg shadow-purple-500/20 transition-all active:scale-[0.98] text-sm flex items-center justify-center gap-2">'+
            '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>'+
            t('download')+
          '</button>'+
          '<div id="githubProgress" class="hidden space-y-2">'+
            '<div class="h-2 bg-slate-700 rounded-full overflow-hidden">'+
              '<div id="githubProgressBar" class="h-full bg-purple-500 transition-all duration-300" style="width:0%"></div>'+
            '</div>'+
            '<p id="githubProgressText" class="text-xs text-slate-400 text-center"></p>'+
          '</div>'+
        '</div>'+
        '<div class="rounded-xl bg-slate-800/20 border border-slate-700/20 p-4 space-y-2">'+
          '<h4 class="text-sm font-semibold text-slate-300">📋 '+t('githubNoteTitle')+'</h4>'+
  ...
  checkRateLimit().then(rate => {
    if (!rate) return;
    const el = document.getElementById('githubRateLimit');
    const txt = document.getElementById('githubRateText');
    if (el && txt) {
      txt.textContent = t('githubRateHint').replace('{remain}', rate.remaining).replace('{limit}', rate.limit);
      el.classList.remove('hidden');
      if (rate.remaining < 10) {
        el.classList.remove('bg-amber-500/10', 'border-amber-500/20');
        el.classList.add('bg-red-500/10', 'border-red-500/20');
        txt.classList.remove('text-amber-400');
...
window.startGitHubDownload = async function() {
  ...
  const progressWrap = document.getElementById('githubProgress');
  const bar = document.getElementById('githubProgressBar');
  const txt = document.getElementById('githubProgressText');
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = '<span class="inline-flex items-center gap-2"><span class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span>'+t('downloading')+'</span>';
  }
  if (progressWrap) progressWrap.classList.remove('hidden');
  if (bar) bar.style.width = '0%';
  if (txt) txt.textContent = t('githubFetching');

  try {
    if (info.type === 'blob') {
      const { blob, filename } = await downloadGitHubFile(info, githubAbortController.signal);
      ...
      const onProgress = (done, total, name) => {
        ...
        if (bar) bar.style.width = pct + '%';
        if (txt) txt.textContent = t('githubProgress').replace('{done}', done).replace('{total}', total).replace('{name}', name);
      };
      const { blob, filename, done, total } = await downloadGitHubFolder(info, onProgress, githubAbortController.signal);
      dlBlob(blob, filename);
    ...
    if (err.name === 'AbortError') {
      ...
      toast('❌', err.message || t('downloadFailed'));
    }
  } finally {
    if (btn) {
      btn.disabled = false;
