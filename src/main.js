      ...
      if (catId === 'image') {
        ...
        if (ext === 'zip') {
          if (target === 'zip-extract-normal') {
            blob = await extractZip(item.file, 'normal');
          } else if (target === 'zip-extract-flat') {
            blob = await extractZip(item.file, 'flat');
          } else {
...
window.startExtractMerge = async function() {
  ...
  try {
    const blob = await mergeExtractZips(selected.map(f => f.file));
    dlBlob(blob, 'merged-extract.zip');
