...
import { convArchive, extractZip, mergeExtractZips } from './converters/archive.js';
import { convEbook } from './converters/ebook.js';
import { formatJSON } from './converters/format.js';
import { parseGitHubUrl, isGitHubUrl, checkRateLimit, downloadGitHubFile, downloadGitHubFolder, parseReleaseUrl, isReleaseUrl, fetchReleases, getDefaultBranch } from './converters/github.js';
import { parseGiteeUrl, isGiteeUrl, downloadGiteeFile, downloadGiteeFolder, getGiteeDefaultBranch } from './converters/gitee.js';
...
if (typeof JSZip === 'undefined' && typeof window !== 'undefined' && !window.JSZip) {
  console.error('[FormatHub] 致命错误: JSZip 库未加载，压缩/解压功能将不可用。请检查 index.html 中是否引入了 jszip.min.js');
}
...
async function renderCategoryPage(catId) {
  ...
  let extractMergeBtn = '';
  if (catId === 'data') {
    extractMergeBtn = '<button onclick="window.startExtractMerge()" class="w-full py-2.5 rounded-xl border border-'+cat.color+'-500/30 text-'+cat.color+'-400 hover:bg-'+cat.color+'-500/10 transition-all text-sm font-medium flex items-center justify-center gap-2">'+
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (catId === 'image') {
        ...
        if (ext === 'zip') {
          if (target === 'zip-extract-normal') {
            const r = await extractZip(item.file, 'normal');
            if (r && r.type === 'extracted') {
              showExtractModal(r.entries, item.file.name + ' — ' + t('zipExtractNormal'));
              item.status = 'done'; item.progress = 100;
              renderFiles(catId);
              continue;
            }
          } else if (target === 'zip-extract-flat') {
            const r = await extractZip(item.file, 'flat');
            if (r && r.type === 'extracted') {
              showExtractModal(r.entries, item.file.name + ' — ' + t('zipExtractFlat'));
              item.status = 'done'; item.progress = 100;
              renderFiles(catId);
              continue;
            }
...
function showExtractModal(entries, title) {
  const modal = document.getElementById('extractModal');
  const list = document.getElementById('extractList');
  modal.classList.remove('hidden'); modal.classList.add('flex');
  window._extracted = entries.map(e => ({...e, selected: true}));
  window._extractTitle = title || t('zipExtract');
  renderExtractList();
}

function renderExtractList() {
  const list = document.getElementById('extractList');
  const entries = window._extracted || [];
  const allSelected = entries.every(e => e.selected);
  const selectedCount = entries.filter(e => e.selected).length;

  let html = '<div class="sticky top-0 z-10 bg-slate-800/95 backdrop-blur-sm border-b border-slate-700 pb-3 mb-2">'+
    '<div class="flex items-center justify-between mb-2">'+
      '<h4 class="text-xs font-semibold text-slate-300">'+(window._extractTitle||'')+' · '+entries.length+t('fileCount')+'</h4>'+
      '<span class="text-[10px] text-slate-500">'+selectedCount+'/'+entries.length+t('fileCount')+' '+t('selected')+'</span>'+
    '</div>'+
    '<div class="flex items-center gap-2">'+
      '<button onclick="window.selectAllExtract()" class="text-[10px] px-2 py-1 rounded bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors">'+t('selectAll')+'</button>'+
      '<button onclick="window.deselectAllExtract()" class="text-[10px] px-2 py-1 rounded bg-slate-700 text-slate-300 hover:bg-slate-600 transition-colors">'+t('deselectAll')+'</button>'+
      '<div class="flex-1"></div>'+
      '<button onclick="window.dlExtractSelected()" class="text-[10px] px-2.5 py-1 rounded-md bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 transition-colors font-medium">'+t('downloadSelected')+'</button>'+
      '<button onclick="window.dlExtractAll()" class="text-[10px] px-2.5 py-1 rounded-md bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 transition-colors font-medium">'+t('downloadAll')+'</button>'+
    '</div>'+
  '</div>';
  ...
  for (let i=0; i<entries.length; i++) {
    ...
    html += '<div class="flex items-center gap-2 p-2.5 rounded-lg bg-slate-700/20 hover:bg-slate-700/40 transition-colors">'+
      '<input type="checkbox" onchange="window.toggleExtractSelect('+i+')" '+checked+' class="w-4 h-4 rounded border-slate-600 bg-slate-800 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-0 cursor-pointer shrink-0">'+
      '<div class="min-w-0 flex-1">'+
        '<p class="text-xs truncate '+(e.selected?'text-slate-200':'text-slate-500')+'">'+e.path+'</p>'+
...
window.toggleExtractSelect = function(idx) {
  if (window._extracted && window._extracted[idx]) {
    window._extracted[idx].selected = !window._extracted[idx].selected;
    renderExtractList();
  }
};

window.selectAllExtract = function() {
  if (window._extracted) {
    window._extracted.forEach(e => e.selected = true);
    renderExtractList();
  }
};

window.deselectAllExtract = function() {
  if (window._extracted) {
    window._extracted.forEach(e => e.selected = false);
    renderExtractList();
  }
};
...
window.dlExtractAll = function() {
  const entries = (window._extracted || []).filter(e => e.content);
  if (!entries.length) return;
  // 浏览器并发限制：间隔 300ms 逐个触发下载
  entries.forEach((e, i) => {
    setTimeout(() => {
      dlBlob(e.content, e.path.split('/').pop());
    }, i * 300);
  });
  toast('✅', t('downloadAll') + ' ' + entries.length + t('fileCount'));
};

window.dlExtractSelected = function() {
  const selected = (window._extracted || []).filter(e => e.selected && e.content);
  if (!selected.length) { toast('⚠️', t('noFileSelected')); return; }
  selected.forEach((e, i) => {
    setTimeout(() => {
      dlBlob(e.content, e.path.split('/').pop());
    }, i * 300);
  });
  toast('✅', t('downloadSelected') + ' ' + selected.length + t('fileCount'));
};
...
window.startGitHubDownload = async function() {
  ...
  // 如果没有指定分支，获取默认分支
  if (!info.branch) {
    if (text) text.textContent = '正在获取默认分支...';
  ...
  try {
    if (info.type === 'blob') {
      ...
      if (text) text.textContent = t('githubFetching');
...
window.downloadReleaseAssetByIdx = async function(idx) {
  ...
  if (!asset) return;
  try {
    toast('⏳', t('downloading') + ' ' + asset.name);
    const res = await fetch(asset.browser_download_url);
    if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
    const blob = await res.blob();
    dlBlob(blob, asset.name);
    toast('✅', t('downloadSuccess') + ': ' + asset.name);
  } catch (err) {
    toast('❌', t('downloadFailed') + ': ' + err.message);
  }
};

window.startExtractMerge = async function() {
  const selected = files.filter(f => f.selected && getExt(f.file.name) === 'zip');
  if (selected.length < 2) { toast('⚠️', t('needTwoZip')); return; }
  try {
    const r = await mergeExtractZips(selected.map(f => f.file));
    if (r && r.type === 'extracted') {
      showExtractModal(r.entries, t('extractMergeTitle'));
      toast('✅', t('extractMergeDone'));
    }
  } catch (err) {
    toast('❌', err.message || t('failed'));
  }
